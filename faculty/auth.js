...
function renderUserProfile(user) {
    if (!user) return;
    const nameStr = [user.first_name, user.last_name].filter(Boolean).join(' ') || user.username || 'Faculty Member';
    const roleStr = user.role_name || user.role_code || 'Faculty Member';
    ...
    if (nameNode) nameNode.innerText = nameStr;
    if (roleNode) roleNode.innerText = roleStr;
    if (avatarNode) avatarNode.innerText = nameStr.charAt(0).toUpperCase();

    const badge = document.getElementById('user-profile-info');
    if (badge) {
        badge.innerHTML = `
            <span class="role-pill">${roleStr}</span>
            <span style="font-size:14px;font-weight:600;margin-right:15px;">${nameStr}</span>
        `;
    }
}
